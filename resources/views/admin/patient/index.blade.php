...
@section("content")
<div class="mb-3">
    <a class='btn btn-success' href='{{route("patient.create")}}'>اضافة مريض</a>
</div>
<form>
    <div class="row ">
    ...
    <table class='table table-hover'>
        ...
        @foreach($items as $item)
            <tr>
                ...
                <td>
                    <form method='post' action="{{route('patient.destroy',$item->id)}}">
                        @csrf
                        @method('delete')
                        <a href='{{route("patient.show",$item->id)}}' class='btn btn-info'>عرض</a>
                        <a href='{{route("patient.edit",$item->id)}}' class='btn btn-primary'>تعديل</a>
